...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let release_note_path = format!(
        "releasenotes/notes/bump-mcapi-{}.yaml",
        &version_hash[..16]
    );
    let release_note = ReleaseNote {
        fixes: vec![format!(
            "The Cluster API driver for Magnum has been bumped to {} to improve stability, fix bugs and add new features.",
            version
        )],
    };
    fs::write(&release_note_path, serde_yaml::to_string(&release_note)?).await?;
