...
{{- /*
    ...
    Usage:
        range $name := rest (splitList include "manila.utils.ceph_backend_list" $)
*/ -}}
{{- define "manila.utils.ceph_backend_list" -}}
  {{- range $name, $backend := .Values.conf.backends -}}
    {{- if kindIs "map" $backend }}
      {{- if (eq $backend.volume_driver "manila.volume.drivers.rbd.RBDDriver") -}}
        {{- "," -}}
        {{- $name -}}
      {{- end -}}
    {{- end -}}
