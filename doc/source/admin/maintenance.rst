...
.. admonition:: Note
    ...
    It's generally not recommended to use the ``nova`` client however the
...
re-enable the OpenStack compute service by running the following command:
...
.. admonition:: Note
    ...
    VMs but it won't automatically move the VMs back to the node. You will
