...
If active sessions remain, disable fails safely by default. To drain them
through Ironic before removing the console resources, set
``ironic_vnc_disable_mode: drain``. The role also checks namespaces listed in
``ironic_vnc_previous_namespaces`` when cleaning up older deployments.
