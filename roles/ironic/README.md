...
`ironic_vnc_token_timeout` defaults to Ironic's
600-second lifetime; increase it only when the additional bearer-token exposure
is acceptable for the operator workflow. By default,
`ironic_vnc_disable_mode: fail` refuses feature disable or namespace migration
while provider-owned Pods or Secrets remain. Set `ironic_vnc_disable_mode: drain`
to disable affected node consoles and wait for their Pods and Secrets to
disappear. Cleanup checks the deployed namespace, the configured namespace,
and any entries in `ironic_vnc_previous_namespaces`.
